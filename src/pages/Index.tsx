...
const Index = () => {
  ...
  const downloadContract = () => {
    ...
    const contractText = `
ДОГОВОР ЗАЙМА №${Math.floor(Math.random() * 10000)}

г. Москва                                                     ${contractData.contractDate}

ЗАИМОДАВЕЦ: ООО "МикроФинанс", именуемый в дальнейшем "Заимодавец"

ЗАЕМЩИК: ${contractData.fullName}
...
1. ПРЕДМЕТ ДОГОВОРА
1.1. Заимодавец передает Заемщику денежные средства в размере ${loanAmount.toLocaleString('ru-RU')} рублей, 
а Заемщик обязуется возвратить указанную сумму и уплатить проценты в размере и сроки, 
установленные настоящим Договором.

2. СУММА И СРОК ЗАЙМА
2.1. Сумма займа: ${loanAmount.toLocaleString('ru-RU')} рублей
2.2. Срок займа: ${loanDays} дней
2.3. Процентная ставка: 2% в день
2.4. Сумма к возврату: ${contractData.totalAmount.toLocaleString('ru-RU')} рублей

3. ПОРЯДОК ВОЗВРАТА ЗАЙМА
3.1. Заемщик обязуется возвратить сумму займа и уплатить проценты в течение ${loanDays} дней 
с момента получения денежных средств.

4. ОТВЕТСТВЕННОСТЬ СТОРОН
4.1. За нарушение срока возврата займа Заемщик уплачивает пени в размере 0,1% 
...
_________________                    _________________
    ...
    link.href = url;
    link.download = `Договор_займа_${contractData.fullName.replace(/\s+/g, '_')}.txt`;
    link.click();
